...
`arunasiweb\セットアップ.bat` をそのままダブルクリックしても構いません。
そのときは入れ子（`arunasiweb\arunasiweb`）を作らず、**そのフォルダ自身を最新にします**。

まだ GitHub に送っていない変更が残っているフォルダでは、上書きしないよう
**更新を止めます**。`プッシュ.bat` で変更を送ってから、もう一度実行してください。
