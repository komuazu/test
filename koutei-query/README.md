# koutei-query — 受注番号から「仕上りサイズ・加工内容・内外作・委託先・用紙」を引く

平版印刷 → オンデマンド移行検討で、`平版印刷_オンデマンド移行検討_3000通し以下一覧.xlsx` の
「仕上りサイズ」「備考」を埋めるための道具。koutei-kanr30（社内工程管理システム）の
**本番 DB（丸本PC）を読むだけ**で、DB もコードも書き換えない。

```
koutei-query/
├── export_finish_size.py     … 本体（丸本PC で動かす）
├── make_excel.py             … 取得結果を元の Excel の月別シートに流し込み、資料の形に整える
├── query_finish_size.sql     … 同じことを psql だけでやる版（検算用）
├── tests/fixture.sql         … 動作確認用のダミーデータ（本番に流さない）
├── tests/run_test.py         … 動作確認（Python 版と SQL 版の両方）
├── tests/compare_on_dump.py  … 本番ダンプで Python 版と SQL 版を全件突き合わせる
└── tests/test_excel.py       … make_excel.py の動作確認（DB 不要）
```

---

## 1. 先に知っておくこと（調べて分かったこと）

### 製造指示書・印刷指示書の PDF は koutei-kanr30 が作ったものではない

Google ドライブの `作業依頼書` フォルダにある「【製造指示書】」PDF（例: 08728296 障害者採用パンフレット）は
**基幹の受注・生産管理システム（MIS）の帳票**で、`部品／内・外作／加工所／加工日程／加工作業／台数／仕上サイズ／展開サイズ`
の表もそちらのもの。koutei-kanr30 のコード（`komuazu/koutei`）には、この表を持つテーブルも、
「製造指示書」「印刷指示書」を出力する処理も存在しない（あるのは「外注委託依頼書」だけ）。

koutei-kanr30 が持っているのは、MIS の CSV から取り込んだ項目と、画面で手入力した項目に限られる。

| 欲しい項目 | koutei-kanr30 での在りか | 元 |
|---|---|---|
| 仕上りサイズ | `timeline_processes.data` / `waiting_list.data` の JSON `finish_size`（最上位と `events[]` の要素）、`outsourcing_list.finish_size`、`delivery_schedule.finish_size` | MIS CSV の「製品仕上サイズ名」（`calendar_ui.html` の取込）または手入力 |
| 加工内容 | JSON `finish_processing_name` / `finish_processing` / `finishProcessing` / `processing_content` / `finish_process`（最上位と `events[]`）、`outsourcing_items[].processing_content`、`outsourcing_list.processing_content`、`delivery_schedule.processing_content`、`processing_works.processing_content` / `classification` | MIS CSV の「仕上加工名」または手入力 |
| 内作／外注 | JSON `internal_work` / `outsourcing` / `work_department`、`processing_works` に行があれば内作、`outsourcing_list` に行があれば外注 | 画面のチェック |
| 委託先名 | JSON `outsourcing_company` / `outsource_name` / `outsourcing_items[].company`、`outsourcing_list.outsourcing_company`、`delivery_schedule.outsourcing_company` | MIS CSV の「外注先名」または手入力 |
| 用紙銘柄・規格・斤量 | JSON `paper_type` / `standard_size` / `paper_weight`、部品ごとの `order_paper_info[]`・`papers[]` の同名キー | 画面の用紙選択（`paper_master` から） |

**MIS から取り込んだ「仕上加工名」は `data.events[]` の中にしか残らない**（`calendar_ui.html` 5295〜5309 行で最上位へ
写しているのは `finish_size` と `outsource_name` だけ）。2025-12 のダンプでは、最上位の `finish_process` は 0 行、
`events[].finish_process` に値があるのは 45 受注だった。スクリプトは最上位と `events[]` の両方を読む。

つまり **加工所（第二工場など）・加工日程・台数は「外注委託依頼書」を出した案件と、内作加工を登録した案件にしか入っていない。**
MIS の製造指示書と同じ粒度がどうしても要る項目は、MIS 側から取る必要がある。

### 「仕上りサイズ B1」は MIS 一括取込の既定値の疑いがある

2025-12 のダンプでは、`waiting_list` の `finish_size = 'B1'` が 874 行あり、**全部が同じ分（2025-12-12 09:52）の
一括取込**で、受注数量「1」が 768 行、仕上加工名は「-」だった。手入力側（タイムライン）の仕上りサイズは A4 が最多で
B1 はほぼ無い。MIS の「製品仕上サイズ名」が未入力のとき B1 になっていないか、MIS 側で確かめること。
スクリプトは B1 だけの案件の件数を末尾に出す。詳細 CSV の「作成日時」で一括取込かどうか見分けられる。

### 桁数の違う受注番号は拾えない

`087033142②`（8 桁）、`0870379(1)`（6 桁）のように、桁数を打ち間違えた行がダンプに 8 行ある。
先頭の数字全体で比べるので、これらはどの受注番号にも当たらず出力に出ない（`087033142②` は中身からして
`08703314` と同じ案件に見えるが、機械的につながない）。必要なら手で直す。

### 受注番号には枝が付いていることがある

koutei には `08701248（表紙）`、`08704276②`、`08704276③（複製1）`、`08702636複製1` のように、部品や複製で
受注番号の後ろに文字が付いた行がある（ダンプで 283 行・87 受注。うち 8 受注は素の番号の行が無い）。
**先頭の数字だけで突き合わせ**、枝は詳細 CSV の「枝」列に残す。本体 CSV では部品ぶんの値を ` / ` でつなぐ。
※ ② のような丸数字は NFKC 正規化すると「2」になって別の受注番号に化けるので、正規化は数字部分の全角→半角だけにしている。

### 本番 DB は丸本PC の中からしか見えない

`web_app/.env.marumoto` の接続先は `localhost:5432`、DB 名は `koutei_kanri_dev`（本番機でもこの名前）。
PostgreSQL は丸本PC の自分自身にしか口を開けていないので、**このスクリプトは丸本PC の上で動かす。**
クラウド側の Claude Code からは届かない（2026-09-17 に確認済み）。

### 受注番号一覧 CSV は同梱されていなかった

`仕上りサイズ未確認_受注番号一覧.csv` はこのリポジトリにも Google ドライブにも無かった。
手元のものを丸本PC に置いて `--orders` で渡す。

---

## 2. 丸本PC での動かし方

前提: koutei-kanr30 が動いている PC（`C:\Users\116544\Desktop\UPDATA17` に本体がある）。
Python と `psycopg2` は koutei 本体が使っているものがそのまま使える。

```bat
cd /d <このフォルダ>
python export_finish_size.py --orders 仕上りサイズ未確認_受注番号一覧.csv ^
    --env C:\Users\116544\Desktop\UPDATA17\web_app\.env
```

**受注番号一覧の CSV が用意できないときは `--all`。** DB にある受注番号を全部出す。
絞り込みは後でこちら側でやればよい（本番ダンプ 2,164 件で、一覧を渡したときと
1 セルも違わないことを確かめている）。

```bat
python export_finish_size.py --all --env C:\Users\116544\Desktop\UPDATA17\web_app\.env
```

* `--env` には koutei が使っている `.env`（`DATABASE_HOST` などが書いてあるファイル）を渡す。
  省略すると、カレント／`web_app/` の `.env` → `.env.marumoto` の順に探す
* パスワードは `.env` の `DATABASE_PASSWORD` を使う。無ければその場で聞く。**画面にもファイルにも出さない**
* 接続は `default_transaction_read_only=on` で開く。うっかり書き込み文を流しても DB 側で拒否される
  （`tests/run_test.py` で DELETE が拒否されることを確かめている）
* 受注番号一覧は 1 列目、または「受注番号」「管理番号」を含む見出しの列を読む。
  UTF-8 でも CP932（Excel の CSV 保存）でもよい。数字で始まらない値（空欄・「0」など）は飛ばして件数を出す

### 受注番号のゆれ

| 表記 | 例 | 扱い |
|---|---|---|
| PDF・koutei | `08726258` | 前ゼロを落として `8726258` |
| Excel | `8726258` / `8726258.0` | `.0` と前ゼロを落として `8726258` |
| 全角・空白入り | `８７２６２５８`、`8726 258` | 数字を半角にし、空白を除く |
| 枝付き（koutei 側） | `08726258（表紙）`、`08726258②` | 先頭の数字 `8726258` で突き合わせ、枝は詳細 CSV に残す |

DB 側も同じ規則で比べるので、どちらの表記で入っていても拾う。

---

## 3. 出力

### `仕上りサイズ_koutei取得結果.csv`（Excel にそのまま貼る用）

```
受注番号, 仕上りサイズ, A3以下, 加工内容, 内外作区分, 委託先名, 用紙銘柄, 用紙規格, 斤量
```

* 依頼メモの 5 列に、「A3以下」と用紙の 3 列を後から足した。要らなければ Excel 側で読み飛ばせばよい
* **A3以下** は仕上りサイズの表記から機械的に判定した `○`（A3 以下）/ `×`（A3 より大きい）/ `不明`
  * 実寸（`297×210`、`153*75`、`29.7×21cm`）は 297×420mm（＋3mm まで）に収まれば ○
  * 規格名は A3〜A8・B4〜B8 が ○、A0〜A2・B0〜B3・A全／菊全／四六／A3ノビ が ×。ハガキ・名刺・長3・角2 などは ○
  * 「規格外」だけで実寸が無いものは 不明。仕上りサイズが無い受注も 不明
  * 部品が複数あるときは **1 つでも × なら ×**、○ が 1 つでもあって × が無ければ ○
* `仕上りサイズ_koutei取得結果_A3以下.csv` に、判定 ○ の行だけを同じ列で出す。`--a3-only` を付けると本体 CSV も ○ だけになる
* 判定は Python 版だけ。SQL 版には無い
* 入力 CSV の並びのまま、**受注番号は入力の表記のまま**返す（XLOOKUP のキーにそのまま使える）
* 該当が無い受注番号は空欄
* 同じ受注番号に複数行（本体／表紙などの部品、返し・続きイベント、枝付き）があるときは、
  重複を除いて ` / ` でつなぐ。並びは読んだ順（テーブルの順 → 行 ID 順）で固定
* 内外作区分は `内作` / `外注` / `内作・外注`（両方ある）/ 空欄（判定材料なし）
  * 内作 = 内作チェックあり、内作加工の作業一覧に行あり、または加工所が 第二工場／本社／POP課
    （「営業」「その他」「-」は内作と判定しない）
  * 外注 = 外注チェックあり、外注依頼書あり、または委託先名あり（「未手配」は委託先と数えない）
* 「-」「未設定」「未定」「なし」は値なし扱い。「その他」（MIS の仕上加工名、`processing_works` の区分）は加工内容に出さない
* `data.events[]` の要素に別の受注番号が書いてあるもの（配置時のコピーで持ち越された行。ダンプで 2 行）は読まない。
  詳細 CSV の「返し/続き」列に `events:<番号>` と出る
* 受注番号として丸ごと読めない値（`8726274.5`、`0`、`2026年9月 一覧`）は飛ばして件数を出す
* 委託先名の表記ゆれ（`㈱松岡製本`／`松岡製本（株）`／`松岡製本`）はそのまま別名で並ぶ。名寄せはしない
* UTF-8（BOM 付き）、CRLF。Excel でそのまま開ける

### `仕上りサイズ_koutei取得結果_A3以下.csv`

本体と同じ列で、判定が ○ の受注番号だけ。オンデマンド移行候補の絞り込みにそのまま使う。
「不明」は入っていないので、規格外で実寸が無い案件は本体 CSV で別途見る。

### `仕上りサイズ_koutei取得結果_詳細.csv`（検算用）

どのテーブルのどの行から何を取ったかを 1 行ずつ。「枝」「部品」「作成日時」があるので、
部品ごとの値の切り分けと、B1 の一括取込の見分けに使う。`委託先ごとの加工内容` は
`八王子紙工：ミシン(筋)` の形で、備考欄の文面を作るときに使える。

---

## 3.5 Excel に流し込む（`make_excel.py`）

取得結果 CSV を、元の Excel（`平版印刷_オンデマンド移行検討_3000通し以下一覧.xlsx`）の月別シートに流し込み、
資料として見られる形に整える。DB は使わないので、どの PC でも動く（openpyxl が要る）。

```bat
python make_excel.py --base 平版印刷_オンデマンド移行検討_3000通し以下一覧.xlsx --result 仕上りサイズ_koutei取得結果.csv
```

できるもの: `平版印刷_オンデマンド移行検討_3000通し以下一覧_koutei反映.xlsx`（**元のファイルは書き換えない**）

| すること | 中身 |
|---|---|
| 月別シートを見つける | シート名に「月」があり、先頭 15 行に「受注番号」か「管理番号」の見出しがあるシート。サマリーなど他のシートは元のまま |
| 空欄だけ埋める | 「仕上りサイズ」「備考」が空の行だけ、koutei の値で埋めて **水色** に塗る。手で記入済みの 46 件は触らない |
| 備考の文面 | `外注（松岡製本）：抜き・ポケット貼り・24P中綴じ`、`内作：折加工（二つ折り）`、`内作・外注（八王子紙工）：中綴じ12P / ミシン(筋)` の形 |
| 右端に列を足す | A3以下(koutei判定)・加工内容・内外作・委託先名・用紙銘柄・用紙規格・斤量・仕上りサイズ(koutei)。見出しは濃い青 |
| 見た目 | 見出し行を紺色、罫線、1 行おきの薄い灰色、列幅、ウィンドウ枠の固定、オートフィルタ、A4 横 1 枚幅の印刷設定。A3以下は 緑=○／橙=×／黄=不明 |
| 集計シート | 「集計（koutei）」を末尾に足す。A3以下と内外作の件数は COUNTIF の式で、月シートを直せば追従する。凡例と注意書き付き |

* 受注番号は `8726258`／`08726258`／`8726258.0` のどれで入っていても突き合わせる
* `--base` を省くと、取得結果だけから「一覧」シートの Excel を作る（月別には分かれない）
* フォントは Meiryo。表題は見出し行の上が空いているときだけ入れる

---

## 3.6 DB を使わずに作る（帳票 PDF から。実際に納品した方法）

丸本PC の DB に届かなかったので、**Google ドライブ「作業依頼書」「委託業務依頼書」フォルダの帳票 PDF**
（製造指示書・印刷指示書・外注委託依頼書）の文字を読んで作った。2026-09-17 の実績:

| 手順 | 道具 | 結果 |
|---|---|---|
| ドライブの PDF の文字を取り出す（読むだけ） | 月ごとに並列のエージェント（`mcp__Google_Drive__read_file_content`）→ `pdftext/<fileId>.json` | 1,624 通（製造指示書 1,141・外注委託依頼書 387・印刷指示書 32） |
| 帳票の文字から項目を切り出す | `shiji_parse.py` | 仕上りサイズ・受注数量・総頁数・色数・用紙（銘柄／規格／連量）・通し数（計画）・加工（内外作／加工所／作業／原文）・出荷日 |
| 母集団（全社印刷実績の通し数 3,000 以下 796 行）に管理番号で付けて月別 Excel に | `build_final.py --base 全社印刷実績.xlsx --textdir pdftext` | 帳票あり 483 行（6 月以降はほぼ全件、4〜5 月は 2 割） |

```bat
python build_final.py --base 2026年9月_全社_印刷実績.xlsx --textdir pdftext --out 平版印刷_オンデマンド移行検討_3000通し以下_資料.xlsx --csv 同名.csv
```

* `build_from_pdf.py` は母集団を使わず、帳票のある受注を全部並べる版（出荷日の月でシート分け）
* PDF の文字は表の枠が消えて 1 行に潰れている。Drive が Markdown 風（セルごとに 1 行）で返すものも
  99 通あるので、**全文を 1 行につないでから見出し語で切り出す**。「再」発行の帳票は見出しブロックが
  途中に挟まる、様式によっては「受注日 入稿日 下版日」が「前回受注番号」より先に来る、などの並びの違いがある
* 用紙規格は「四六全判」「４／６判縦」「四六半」「A全判」など。寸法の「×」は `\*` で来ることがある
* 通し数の見出しが 2 段に割れる様式（色数の後ろに見出し語が挟まる）があるので、見出し語を全部落としてから数字を読む
* 得意先名と製品名の切れ目は、ファイル名の品名で割る。ファイル名と本文の受注番号が違う帳票が 56 通あったので、**本文の受注番号**で突き合わせる
* 第三者検証（30 行の目視突き合わせ、取りこぼしの機械スキャン、集計式、A3 判定）の指摘はすべて反映済み。
  残る取りこぼしは 製造指示書 1,141 通のうち 仕上りサイズ 5・通し数 6・用紙 12（OCR 崩れ・行が逆順の PDF など）

### 帳票が無い 313 行を埋める

帳票は **6 月以降に急に増えている**（ドライブの PDF は 3 月 107・4 月 88・5 月 81 に対し
6 月 404・7 月 365・8 月 293・9 月 291 通）。埋まらない 313 行のうち 261 行が 4〜5 月で、
**そもそもドライブに帳票が無い**。埋める道は 2 つある。

```bat
rem 1) koutei の DB から（確かな値）。丸本PC で export_finish_size.py を走らせた CSV を渡す
python build_final.py --base 全社印刷実績.xlsx --textdir pdftext --koutei-csv 仕上りサイズ_koutei取得結果.csv --out 資料.xlsx

rem 2) 再版の帳票からの推定（確かな値ではない）。79 行が埋まる
python build_final.py --base 全社印刷実績.xlsx --textdir pdftext --guess-reprint --out 資料_推定込み.xlsx
```

* **優先順は 帳票 → DB → 推定。**帳票がある管理番号は上書きしない。ただし
  **帳票で空だった欄だけは DB の値で補う**（「元の帳票」列が `製造指示書・koutei(DB)` のように並ぶ）。
  実データでは 303 件が該当し、内訳は 内外作 72・用紙規格 3・用紙銘柄 2・連量 2・仕上りサイズ 1・加工内容 1・加工所 1
* `--exclude-name 本機校正` で品名にその語を含む行を外す。本刷りではないため（実データで 52 行・通し数 1,350）
* `--blank-process 化粧断裁` で加工内容が空の行にその語を入れる。加工が無い＝化粧断裁だけ、という扱い。
  **帳票にも DB にも行が無い行には入れない**（実データで 5 行はそのまま空欄）
* `--conflict-size DB` で、判定が割れたときに DB の仕上りサイズを載せる（既定は `帳票`）。
  どちらを選んでも「要確認」シートに両方の値と「採用」列が残る
* `--conflict-keep-doc 中綴じ --conflict-keep-doc 無線綴じ` で、その語を加工内容に含む行だけ帳票を採る。
  **綴じものは DB に折る前の見開き寸法が混ざる**ため（例 08726258「抜き・ポケット貼り・24P中綴じ」は
  帳票が『A4 297×210』、DB が『433*427 / A4』。433×427 は折る前の寸法）。実データで 5 件
* `--force-a3 8718670` で、その管理番号を A3 以下（○）として扱う。**MIS 一括取込の既定値「B1」**が
  入っていて機械判定が実物と合わない行の手当て。「元の帳票」列に `手当て` が並ぶので後から追える。
  実データでは仕上りサイズが「B1」だけで外れた 17 行のうち、品名と合わないのは 2 行
  （08718670「ブラジル大使館B6冊子 表紙」、08722015「ABの自習シート」）
* `--blank-inout 内作` で内外作が空の行を内作にする。**外注一覧に記録が無ければ内作**という扱い。
  委託先が入っている行はそのまま（実データでは空欄 464 行すべて委託先も空だった）
* **帳票と DB で A3 の判定が割れた行は「要確認」シートに両方の値を並べる。**
  本番 DB（2026-09-18、受注 4,277 件）と帳票 1,152 件を突き合わせたところ一致は 93.9%。
  割れ方には向きがあり、**帳票側は用紙の大きさを仕上りサイズ欄に拾う**ことがあり
  （例 08723108『規格外 788×1091』＝四六全判、DB は『B4』）、**DB 側は別部品や
  MIS 一括取込の既定値「B1」を並べる**ことがある（例 08718340『A3』に対し DB は
  『A3 / A1 / 120*120 / 300*900 / B1』）。どちらが正しいとは決めず、元を見て決める
* 出どころは「元の帳票」列（`koutei(DB)` / `再版推定 nnnnnnn`）と地の色（水色／薄紫）で分かる。
  集計シートにも「うち帳票／うち DB／うち再版推定」の内訳が出る
* **再版推定はあくまで推定。**帳票の「前回受注番号」を逆引きして、その再版の仕様を写しているだけ。
  実データで元も再版も帳票がある 473 組を突き合わせた一致率:

  | 項目 | 一致 |
  |---|---|
  | A3 以下の判定 | 452 / 469（96.4%、不明を除く） |
  | 仕上りサイズ | 386 / 469（82%） |
  | 用紙銘柄 | 388 / 466（83%） |
  | 連量 | 370 / 466（79%） |
  | 加工内容 | 276 / 414（67%） |

  食い違いの多くは、**連番の別部品を前回受注番号に入れている帳票**（ポスターとリーフレットなど）。
  例: 08727990『A3 420×297』の再版とされる 08727991 は『規格外 625×880』。**決める前に元を見ること**

  受注番号の差で分けると、連番ほど当たらない。

  | 元と再版の番号の差 | 組数 | A3 判定の一致 | 仕上りサイズの一致 |
  |---|---|---|---|
  | 1〜3（別部品の疑い） | 72 | 88.9% | 73.6% |
  | 4〜50 | 52 | 96.2% | 82.7% |
  | 51〜 | 349 | 98.0% | 84.1% |

  実際に埋まる 79 行を調べたところ **差 1〜3 は 0 件**（差 51〜 が 77 件、差 4〜50 が 2 件）だったので、
  今のところ危ない側は混ざっていない。母集団が変わったら数え直すこと

* ちなみに「帳票なし」312 件のうち 80 件は、**他の帳票の前回受注番号としてだけ**ドライブに出てくる。
  受注番号として読める帳票は 1 通も無いので、読み取り漏れではなく本当に帳票が無い
* `--csv` の先頭に **`シート` 列**がある（`2026-04` … / `未判定`）。`--a3-only` のとき
  未判定シートの行も CSV に出すため。明細の中身は列が 1 つ増えただけで変わらない

### 第三者検証で直したこと（2026-09-17）

| 指摘 | 直し方 |
|---|---|
| `--a3-only --csv` で未判定シートの行が CSV から抜けていた（168 行）| Excel と同じシート一覧を CSV も回る。先頭に `シート` 列 |
| 複数部品の仕上りサイズ（`A4 … / A2 …`）を 1 つとして判定していた | `judge_cell()` で「 / 」に分けてから渡す。1 部品でも大きければ × |
| 「うち帳票」を `件数 − 空欄 − DB − 推定` で出しており、`A3以下` 列に `帳票なし` が入ると **−108** になった | 「空欄」を `元の帳票` 列が空かどうかで数える。1 つの列なので二重に数えられない |
| CP932 の CSV でトレースバック | UTF-8 → CP932 の順に試し、だめなら理由を出して止まる |
| 値に制御文字があると `wb.save` で落ちる | `clean_cell()` で外す |
| `A3以下` 列に `はい` などが入ると素通り | `○ × 不明` 以外は仕上りサイズから出し直す |
| 同じ受注番号が 2 度あると黙って消える | 後の行を採り、件数を警告に出す |
| 母集団が空だと合計行が `=SUM(B5:B4)` で `#VALUE!`（以前からの不具合）| 月が無いときは 0 を入れる |
| `--a3-only` の合計が母集団と合わない断りが無い | 集計シートの注記に足した |
| 96.4% の分母が文中でずれていた | 「両方に仕上りサイズがある 469 組で 96.4%」に統一 |

2 回目の検証で残っていた穴も直した。

| 指摘 | 直し方 |
|---|---|
| 制御文字を外すときに**改行まで落として**いた（`折り\nミシン` → `折りミシン`）| `openpyxl` の `ILLEGAL_CHARACTERS_RE` に合わせ、改行・復帰・タブは残す |
| 仕上りサイズの区切りが `" / "` ちょうどでないと分けられない | `[/／]` と前後の空白で分ける |
| 制御文字の隣の空白が残る | 制御文字を外してから `strip()` |
| 見出しが違う CSV を黙って 0 件で通す | 「受注番号」が無ければ止まる。ほかの列が無ければ警告 |
| 重複の勝ち負けが `make_excel.py` と逆だった | **先勝ち**にそろえた（警告は残す）|
| ファイルを `with` で開いていない／集計 A2 の文言／印刷日が空のとき CSV の月が空 | それぞれ直した |
| テストのコメントが実際の判定とずれたまま（「直した」と報告して直っていなかった）| `A3以下 以外が全部空 → 埋めない` に直した |

検証で「確かめて問題なし」とされた主なもの: 既定出力の完全一致（796 行 × 23 列、値・色・
フォント・罫線・リンクまで）、`COUNTIF` の `koutei(DB)` の括弧はワイルドカードではない、
集計 16 列 × 全月を 5 パターンで数え直して一致、色とリンクの整合 796 行で不整合 0、
2 回ビルドして byte 一致、循環参照・自己参照 0 件、`clean_cell` が `㈱` や全角・
サロゲートペアを壊さない、見出しを持つ CP932 の CSV が UTF-8 として誤読される道は無い
（`受注番号` の先頭バイト `0x8e` は UTF-8 の開始バイトになり得ない）。

---

## 4. 動作確認

本番に触らずに確かめるには、空の PostgreSQL に `tests/fixture.sql` を流して走らせる。

```bash
python tests/run_test.py --dsn "host=localhost port=5432 dbname=koutei_test user=koutei_dev"
```

* DB 名に `test` が入っていないと止まる（本番に fixture を流さないための安全弁）
* fixture の列定義は本番ダンプ（koutei リポジトリ `web_app/data/pg_backup.sql`、2025-12-15）の
  `CREATE TABLE` を写している（`delivery_schedule.outsourcing` が TEXT、など）
* 8 件の受注番号（ゼロ埋め・小数・全角・枝付き・該当なし・壊れた JSON・「-」「未手配」を含む）で 8 列を突き合わせ、
  SQL 版も同じ結果になることを見る

本番ダンプを別のテスト DB に読み込んで、Python 版と SQL 版を全件（2,164 受注）突き合わせるには:

```bash
psql -d koutei_dump_test -f <koutei>/web_app/data/pg_backup.sql
python tests/compare_on_dump.py --dsn "host=localhost port=5432 dbname=koutei_dump_test user=koutei_dev"
```

Excel 流し込みは DB 無しで確かめられる（元 Excel のダミーを作って流し込み、記入済みの欄が残ることと集計の式を見る）:

```bash
python tests/test_excel.py           # make_excel.py（元 Excel への流し込み）
python tests/test_build_final.py     # build_final.py の --koutei-csv / --guess-reprint
```

2026-09-17 の結果: 差 0 件（SQL 版が「その他」を出すだけの既知の差 16 件を除く）。
内訳は 仕上りサイズあり 2,099／加工内容あり 437／用紙銘柄あり 917／B1 だけ 892。

### SQL 版（`query_finish_size.sql`）の注意

**Python 版が正。SQL 版は検算用**で、2 回目の検証後の直しは SQL 版に入れていない。

```bat
psql -h localhost -p 5432 -U koutei_dev -d koutei_kanri_dev -v ON_ERROR_STOP=1 --csv -q ^
     -c "CREATE TEMP TABLE target(order_no text)" ^
     -c "\copy target FROM '仕上りサイズ未確認_受注番号一覧.csv' WITH (FORMAT csv, HEADER true, ENCODING 'SJIS')" ^
     -f query_finish_size.sql -o 仕上りサイズ_sql版.csv
```

* `-q` を付けないと出力ファイルの先頭に `CREATE TABLE`／`COPY`／`SET` の 3 行が混ざる
* `pg_input_is_valid()` を使うので PostgreSQL 16 以降（本番は 18）
* 5 テーブルが全部ある前提。無いテーブルがあるとエラーになる（Python 版は飛ばす）
* 一時テーブル `target` を作ってから `SET default_transaction_read_only = on` の順（上の手順どおり）
* ` / ` でつないだ値の並びは文字コード順で、Python 版（読んだ順）と違う。中身は同じ
* **Python 版と違う点（既知）**
  * MIS の仕上加工名「その他」をそのまま加工内容に出す（Python 版は出さない）。ダンプで 16 受注。
    テストはこの差を数えない
  * `events[]` の要素に別の受注番号が書いてあっても飛ばさない（Python 版は飛ばす）。ダンプでは 2 行あるが出力には影響なし
  * 値の前後のタブ・全角空白を落とさない、`internal_work` が `"yes"` などのとき真にしない、
    JSON の値が配列やオブジェクトのときそのまま文字列で出す。いずれも実データには無い型

---

## 5. 決めごと

* **本番 DB に書かない。** 接続は読み取り専用で開く。`fixture.sql` は `DROP TABLE` を含むので本番には流さない
* **koutei に無いものは無いと言う。** 加工所・加工日程・台数は外注委託依頼書／内作加工を登録した案件にしか無い。
  空欄が多い項目は「入力されていない」のであって、スクリプトの取りこぼしとは限らない。
  疑うときは `_詳細.csv` で行単位に戻って見る
* **受注番号で機械的につなぐだけ。** 得意先名や品名での名寄せはしない
* **直したら `run_test.py` と `compare_on_dump.py` を通し直す。** Python 版と SQL 版がずれたまま
  「検算用」と言わない
